// ╔══════════════════════════════════════════════════════════════╗
// ║  SURVEYAI — CAPA 3: APP ENCUESTADOR — VERSIÓN DEFINITIVA    ║
// ║  Carga desde Firebase · Agente Local · Flujo completo        ║
// ╚══════════════════════════════════════════════════════════════╝
...
// Cola offline
const Q = {
  add:(i)=>{ try{ const q=JSON.parse(localStorage.getItem("sai_q")||"[]"); q.push({...i,local_id:`l-${Date.now()}`}); localStorage.setItem("sai_q",JSON.stringify(q)); }catch{} },
...
const Stats = {
  ...
  update:(d)=>{ const s=Stats.get(); Stats.save({hoy:s.hoy+1,total:s.total+1,descartes:d?s.descartes+1:s.descartes}); },
...
function PantallaJornada({user,encuesta,onStart}){
  ...
  const iniciar=()=>{
    ...
    try{localStorage.setItem("sai_jornada",JSON.stringify(j));}catch{}
    onStart(j);
  };
    ...
    <div style={{minHeight:"100vh",background:T.bg,display:"flex",flexDirection:"column",
      ...
      <div style={{width:"100%",maxWidth:420}}>
        <div style={{textAlign:"center",marginBottom:24}}>
          <div style={{width:56,height:56,borderRadius:16,background:`${T.green}15`,border:`1px solid ${T.green}30`,
          ...
          <div style={{padding:"9px 13px",borderRadius:10,marginBottom:16,
            ...
            <div style={{display:"flex",alignItems:"center",gap:7}}>
              ...
              <span style={{fontSize:11,color:gps?T.green:T.yellow,fontWeight:600}}>
                {gpsLoad?"Obteniendo GPS...":gps?`GPS ±${gps.acc}m${gps.sim?" (sim)":""}`:  "Sin GPS"}
              </span>
            </div>
      ...
      <div style={{maxWidth:420,margin:"0 auto"}}>
        ...
        <div style={{background:T.card,borderRadius:16,padding:20,border:`1px solid ${T.border}`,marginBottom:14}}>
          <div style={{marginBottom:12}}>
            ...
            <input value={ficha.nombre_anonimo} onChange={e=>set("nombre_anonimo",e.target.value)} placeholder="Anónimo si prefiere"
              style={{width:"100%",background:T.elevated,border:`1.5px solid ${T.border}`,borderRadius:11,
                padding:"11px 13px",color:T.text,fontSize:14,outline:"none",boxSizing:"border-box"}}/>
          </div>
          <div style={{marginBottom:12}}>
              ...
              {["Masculino","Femenino","Otro"].map(g=>(
                <div key={g} onClick={()=>set("genero",g)}
                  style={{padding:"10px",borderRadius:10,cursor:"pointer",textAlign:"center",fontSize:12,fontWeight:600,
                    background:ficha.genero===g?`${T.cyan}15`:T.elevated,color:ficha.genero===g?T.cyan:T.textSec,
...
function PantallaEncuesta({encuesta,jornada,ficha,user,online,onComplete,onDiscard}){
  ...
  const [resp, setResp] = useState(() => {
    const r = {};
    todasLasPreguntas.forEach(p => { r[p._key] = p.tipo==="seleccion_multiple"?[]:""; });
    return r;
  });
  ...
  const p = todasLasPreguntas[paso];
  ...
  const responder = (val) => {
    ...
    if (tipo === "seleccion_multiple") {
      setResp(r => {
        ...
        return {...r, [p._key]: next};
      });
      return;
    }
    setResp(r => ({...r, [p._key]: val}));
    if (p.reglas?.salto_logico?.[val] === "FIN_CON_DESCARTE") {
      const payload = {encuesta_id:encuesta.encuesta_id,encuestador_id:user?.id||"enc-demo",
        es_descarte:true,jornada,ficha,respuestas:{...resp,[p._key]:val}};
      online ? guardarRespuesta(payload).catch(()=>Q.add(payload)) : Q.add(payload);
      Stats.update(true);
  ...
  const enviar = async () => {
    setEnviando(true);
    const paquete = todasLasPreguntas.map((preg,i) => ({
      ...
      metodologia: preg.metodologia||"", respuesta: resp[preg._key]||"",
    }));
    const payload = {
    ...
    if (online) {
      try { await guardarRespuesta(payload); Stats.update(false); setEnviando(false); setExito(true); setTimeout(()=>onComplete(false),2200); }
      catch { Q.add(payload); Stats.update(false); setEnviando(false); onComplete(true); }
    } else {
      Q.add(payload); Stats.update(false); setEnviando(false); onComplete(true);
    }
  };
  ...
  if (exito) return (
    <div style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:24,textAlign:"center"}}>
      <div style={{width:68,height:68,borderRadius:18,background:`${T.green}15`,display:"flex",alignItems:"center",justifyContent:"center",margin:"0 auto 16px"}}>
  ...
  if (!p || total===0) return (
    <div style={{flex:1,display:"flex",alignItems:"center",justifyContent:"center",padding:24,textAlign:"center"}}>
      <div style={{fontSize:13,color:T.textMuted}}>Sin preguntas disponibles</div>
    </div>
  );

  const sesionAnterior = paso>0?todasLasPreguntas[paso-1]._sesionId:null;
            ...
            return(
              <div key={oi} onClick={()=>responder(`opcion_${oi+1}`)}
                style={{padding:"14px 16px",borderRadius:13,cursor:"pointer",
                  background:sel?`${T.cyan}12`:T.elevated,border:`2px solid ${sel?T.cyan:T.border}`,transition:"all .15s"}}>
                <div style={{fontSize:11,fontWeight:700,color:sel?T.cyan:T.textMuted,marginBottom:8,textTransform:"uppercase"}}>
    ...
    if (tipo==="nps") {
      return(
        <div style={{marginBottom:20}}>
          <div style={{display:"flex",justifyContent:"space-between",fontSize:10,color:T.textMuted,marginBottom:8}}>
              ...
              return(
                <div key={n} onClick={()=>responder(String(n))}
                  style={{width:40,height:40,borderRadius:10,cursor:"pointer",
                    ...
                    background:sel?c:T.elevated,color:sel?"#fff":T.textSec,
            ...
            return(
              <div key={i} onClick={()=>responder(v)}
                style={{padding:"11px 15px",borderRadius:12,cursor:"pointer",
                  background:sel?`${T.cyan}12`:T.elevated,border:`2px solid ${sel?T.cyan:T.border}`,
                  display:"flex",alignItems:"center",gap:10,transition:"all .15s"}}>
                <div style={{width:28,height:28,borderRadius:"50%",flexShrink:0,
                  background:sel?T.cyan:T.elevated,border:`2px solid ${sel?T.cyan:T.textMuted}`,
                  display:"flex",alignItems:"center",justifyContent:"center",
        ...
        <div style={{marginBottom:20}}>
          <textarea value={val} onChange={e=>responder(e.target.value)} rows={4}
            ...
            style={{width:"100%",background:T.elevated,border:`1.5px solid ${T.border}`,
              ...
              resize:"vertical",outline:"none",lineHeight:1.7,boxSizing:"border-box",fontFamily:"inherit"}}
            onFocus={e=>e.currentTarget.style.borderColor=T.borderFocus}
            onBlur={e=>e.currentTarget.style.borderColor=T.border}/>
        </div>
      );
          ...
          return(
            <div key={oi} onClick={()=>responder(opt)}
              ...
              <div style={{width:20,height:20,borderRadius:isMulti?5:"50%",flexShrink:0,
                border:`2px solid ${sel?T.cyan:T.textMuted}`,background:sel?T.cyan:"transparent",
    ...
    <div style={{flex:1,display:"flex",flexDirection:"column"}}>
      ...
      <div style={{padding:"10px 18px",display:"flex",justifyContent:"space-between",alignItems:"center",borderBottom:`1px solid ${T.border}`}}>
        <div style={{display:"flex",alignItems:"center",gap:6}}>
          <span style={{fontSize:10,fontWeight:700,color:T.cyan,background:`${T.cyan}15`,padding:"2px 8px",borderRadius:20}}>
            Sesión {p._sesionId}
          </span>
        ...
        <div style={{display:"flex",gap:9}}>
          {paso>0&&(
            <button onClick={()=>setPaso(x=>x-1)}
              style={{flex:1,padding:"13px",borderRadius:12,border:`1px solid ${T.border}`,background:T.elevated,
          ...
          {paso<total-1?(
            <button onClick={()=>{if(puedeAvanzar())setPaso(x=>x+1);}} disabled={!puedeAvanzar()}
              style={{flex:2,padding:"13px",borderRadius:12,border:"none",
                background:puedeAvanzar()?T.grad:T.elevated,
                color:puedeAvanzar()?"#fff":T.textMuted,fontSize:13,fontWeight:700,
                cursor:puedeAvanzar()?"pointer":"not-allowed",fontFamily:"inherit",
            ...
            <button onClick={enviar} disabled={enviando}
              style={{flex:2,padding:"13px",borderRadius:12,border:"none",
                ...
                display:"flex",alignItems:"center",justifyContent:"center",gap:5,
                boxShadow:enviando?"none":"0 4px 14px rgba(16,185,129,0.3)",transition:"all .15s"}}>
...
function PantallaHome({user,jornada,encuesta,stats,online,pendingCount,onIniciar,onCerrarJornada}){
  const sesiones = encuesta?.sesiones
    ...
    <div style={{flex:1,overflowY:"auto",padding:18}}>
      <div style={{background:T.card,borderRadius:14,padding:14,border:`1px solid ${T.cyan}18`,marginBottom:16}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
          {[["📍 Comuna",jornada.comuna],["🏪 Punto",jornada.tipoPunto],["🕐 Inicio",jornada.hora],["🛰️ GPS",jornada.gps?`±${jornada.gps.acc}m`:"Sin GPS"]].map(([l,v])=>(
      ...
      <div style={{background:T.card,borderRadius:16,padding:18,border:`1px solid ${encuesta?T.cyan+"25":T.border}`,marginBottom:14}}>
        {encuesta?(
          ...
          <div style={{fontSize:13,color:T.textMuted,marginBottom:14,textAlign:"center",padding:"12px 0"}}>
            Sin encuesta asignada
          </div>
        )}
        <button onClick={onIniciar} disabled={!encuesta}
          style={{width:"100%",padding:"13px",borderRadius:12,border:"none",
            background:encuesta?T.grad:T.elevated,color:encuesta?"#fff":T.textMuted,
            fontSize:14,fontWeight:700,cursor:encuesta?"pointer":"not-allowed",fontFamily:"inherit",
            display:"flex",alignItems:"center",justifyContent:"center",gap:7,
      ...
      <button onClick={onCerrarJornada}
        style={{width:"100%",padding:"11px",borderRadius:12,border:`1px solid ${T.border}`,background:"transparent",
          color:T.textMuted,fontSize:13,fontWeight:600,cursor:"pointer",fontFamily:"inherit",
          display:"flex",alignItems:"center",justifyContent:"center",gap:7}}>
...
export default function Layer3Encuestador({session,onLogout}){
  ...
  const [screen, setScreen] = useState(()=>{
    if(encIdFromUrl){
      const last=localStorage.getItem("sai_last_enc");
      if(last!==encIdFromUrl){
        localStorage.removeItem("sai_jornada");
        localStorage.setItem("sai_last_enc",encIdFromUrl);
      }
    }
    try{ return JSON.parse(localStorage.getItem("sai_jornada")||"null")?"home":"jornada"; }catch{ return "jornada"; }
  });
  const [jornada,setJornada]=useState(()=>{try{return JSON.parse(localStorage.getItem("sai_jornada")||"null");}catch{return null;}});
  ...
  // Cargar encuesta desde Firebase
  useEffect(()=>{
  ...
  const syncQueue=async()=>{
    const q=Q.get(); if(!q.length) return; setSyncing(true);
    for(const item of q){ try{await guardarRespuesta(item);Q.remove(item.local_id);setPendingCount(Q.count());}catch{break;} }
    setSyncing(false);
  };
  ...
  return(
    <div style={{fontFamily:"'DM Sans',sans-serif",background:T.bg,minHeight:"100vh",color:T.text,maxWidth:480,margin:"0 auto",display:"flex",flexDirection:"column"}}>
            ...
            <div style={{display:"flex",alignItems:"center",gap:7}}>
              ...
              {online&&pendingCount>0&&(
                <button onClick={syncQueue} disabled={syncing}
                  style={{display:"flex",alignItems:"center",gap:4,background:T.cyan,border:"none",borderRadius:7,padding:"3px 9px",color:"#fff",fontSize:10,fontWeight:700,cursor:"pointer",fontFamily:"inherit"}}>
      ...
      {screen==="home"&&jornada&&<PantallaHome user={user} jornada={jornada} encuesta={encuesta} stats={stats} online={online} pendingCount={pendingCount} onIniciar={()=>setScreen("ficha")} onCerrarJornada={()=>{try{localStorage.removeItem("sai_jornada");}catch{}setJornada(null);setScreen("jornada");}}/>}
      {screen==="ficha"&&<PantallaFicha jornada={jornada} onContinuar={f=>{setFicha(f);setScreen("encuesta");}} onCancelar={()=>setScreen("home")}/>}
      {screen==="encuesta"&&encuesta&&<PantallaEncuesta encuesta={encuesta} jornada={jornada} ficha={ficha} user={user} online={online} onComplete={off=>{setStats(Stats.get());setPendingCount(Q.count());setTimeout(()=>{setFicha(null);setScreen("home");},off?400:2400);}} onDiscard={()=>{setStats(Stats.get());setPendingCount(Q.count());setTimeout(()=>{setFicha(null);setScreen("home");},2500);}}/>}
    </div>
  );
}
